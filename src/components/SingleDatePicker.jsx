...
const moment_day=moment.locale() == 'fa' ? momentJalali() : moment();
const defaultProps = {
    ...
    isDayBlocked: () => false,
    isOutsideRange: day => !isInclusivelyAfterDay(day,moment_day ),
    isDayHighlighted: () => {
    },
...
export default class SingleDatePicker extends React.Component {
    constructor(props) {
        ...
        this.today = moment.locale() == 'fa' ? momentJalali() : moment();
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (didFocusChange) {
            values(visibleDays).forEach((days) => {
                Object.keys(days).forEach((day) => {
                    ...
                    const momentObj =moment.locale() == 'fa' ? momentJalali() : moment();
        ...
        const today =moment.locale() == 'fa' ? momentJalali() : moment();
        ...
        if (Object.keys(modifiers).length > 0) {
            ...
            const MonthVisibleDays = getVisibleDays(currentMonth, 1, enableOutsideDays);
            this.setState({
                visibleDays: {
                    ...visibleDays,
                    ...this.getModifiers(MonthVisibleDays)
                },
            });
    ...
    onPrevMonthClick() {
        ...
        const prevMonth = currentMonth.clone().subtract(1, format);
    ...
    onNextMonthClick() {
        ...
        const nextMonth = currentMonth.clone().add(numberOfMonths, format);
    ...
    getFirstFocusableDay(newMonth) {
        ...
        var format = moment.locale() == 'fa' ? 'jMonth' : 'month';
        let focusedDate = newMonth.clone().startOf(format);
        if (date) {
        ...
        if (this.isBlocked(focusedDate)) {
            const days = [];
            const lastVisibleDay = newMonth.clone().add(numberOfMonths - 1, moment.locale()=='fa'?'jMonths':'months').endOf(format);
    ...
    addModifier(updatedDays, day, modifier) {
        ...
        if (enableOutsideDays) {
            const startOfMonth = day.clone().startOf(format);
            const endOfMonth = day.clone().endOf(format);
            if (
                isBeforeDay(startOfMonth, currentMonth.clone().startOf(format)) ||
                isAfterDay(endOfMonth, currentMonth.clone().endOf(format))
            ) {
